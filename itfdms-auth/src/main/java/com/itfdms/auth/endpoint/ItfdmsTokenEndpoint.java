package com.itfdms.auth.endpoint;
...
public class ItfdmsTokenEndpoint {
	 ...
	 */
	@DeleteMapping("/{token}")
	public Result<Boolean> removeToken(@PathVariable("token") String token, @RequestHeader(required = false) String from) {
		if (StrUtil.isBlank(from)) {
			return null;
		}
		return new Result<>(redisTemplate.delete(PROJECT_OAUTH_ACCESS + token));
	}
	 ...
	 */
	@PostMapping("/page")
	public Result getTokenPage(@RequestBody Map<String, Object> params, @RequestHeader(required = false) String from) {
		if (StrUtil.isBlank(from)) {
			return null;
		}
		...
		for (String page : pages) {
			...
			if (authentication instanceof UsernamePasswordAuthenticationToken) {
				...
				if (authenticationToken.getPrincipal() instanceof ItfdmsUser) {
					ItfdmsUser user = (ItfdmsUser) authenticationToken.getPrincipal();
